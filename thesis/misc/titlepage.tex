\thispagestyle{empty}
\begin{center}

\vspace*{1.4cm}
{\LARGE \textbf{Technische Universität Berlin}}

\vspace{0.5cm}

{\large Institut für Telekommunikationssysteme\\[1mm]}
{\large Fachgebiet Architektur der Vermittlungsknoten\\[5mm]}

Fakultät IV\\
Franklinstrasse 28-29\\
10587 Berlin\\
http://www.av.tu-berlin.de\\

\vspace*{1cm}

\includegraphics[width=4cm]{tu_logo.jpg}

\vspace*{0.5cm}

{\LARGE Master Thesis}

\vspace{0.5cm}
{\LARGE \textbf{Design and Implementation of a flexible}}\\
\vspace*{0.3cm}
{\LARGE \textbf{MME Load Balancing solution for Carrier Grade}}\\
\vspace*{0.3cm}
{\LARGE \textbf{Virtual Mobile Core Networks}}\\
\vspace*{1 cm}

{\LARGE Rakesh Varudu}
\\
\vspace*{0.5cm}
Matriculation Number: 0368410\\
08.02.2016\\ % 	date of submission
\vspace*{0.5cm}

Supervised by\\
Prof. Dr. Thomas Magedanz\\
\vspace*{0.3cm}
Assistant Supervisor\\
Prof. Dr. Axel Küpper
\vspace{0.3cm}

\end{center}

